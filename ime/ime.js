...
function match(c, i, p, j) {
	...
	var tmp = '';
	for (var z=i-pos; z<i-pos+p.length; z++) tmp += text[z].unicharAt(0);
...
function autoSelect() {
	for (var i=0; i<text.length; i++) {
		if ($('#sp' + i).hasClass('ok')) continue;

		var t = text[i];
		var c = t.unicharAt(0); //t.charAt(0);
		if (!data[c]) continue;
...
function goNext(step) {
	...
	do {
		...
		c = text[curr].unicharAt(0); //charAt(0);
		d = data[c];
		//d = data[c.charAt(0)];
	} while (! d);
	...
	var tags = d.t ? d.t.split('/') : null;
	for (var i=0; i<d.s; i++) {
		var sl = $('<span data-vs=' + i + '></span>').text(c + (i > 0 ? chr(vsbase+i) : ''));
		if (tags != null && tags[i] != '') $('<span class="tag">'+tags[i]+'</span>').appendTo(sl);
		sl.click(function() {
...
function setEditorText(t) {
	...
	for (var i in text) {
		...
		if (c == '\n') {
			...
			if (data[c.unicharAt(0)]) {
				sp.addClass('p');
				if (text[i].match(/\udb40/)) {
					sp.addClass('ok');
				} else if (data[c.unicharAt(0)].f) {
					sp.addClass('fuzzy');
				}
			}
		}
...
$(window).resize(function() {
	var w = $(window).width();
	var h = $(window).height();
	$('#body').css({width: (w-40)+'px', height: (h-150)+'px'});
	$('#main').css({height: (h-190)+'px'});
	$('#selector').css({width: (w-240)+'px'});

}).resize();

$('#rubyselect').change(function() {
	$('#body').attr('class', 'z' + $(this).val());
}).change();